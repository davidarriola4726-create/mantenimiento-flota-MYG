...
export const ControlMantenimientosView: React.FC<ControlMantenimientosViewProps> = ({
  ...
  // Estado para confirmación de eliminación de mantenimiento
  ...
  // Funciones para Eliminar Registro de Mantenimiento
  ...
  const handleConfirmarEliminarServicio = async () => {
    ...
    try {
      ...
      console.error('Error al eliminar mantenimiento de Firestore:', err);
  ...
  return (
    <div className="space-y-6">
                      ...
                      <tr key={s.id} className="hover:bg-slate-50/80 transition">
                        ...
                        <td className="p-3.5 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            <button
                              type="button"
                              onClick={() => solicitarEliminarServicio(s)}
                              id={`btn-eliminar-mantenimiento-${s.id}`}
      ...
      {/* 6. MODAL DE CONFIRMACIÓN DE ELIMINACIÓN DE MANTENIMIENTO */}
      {servicioAEliminar && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 animate-in fade-in duration-150 print:hidden">
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl border border-red-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              {/* Ficha resumen del mantenimiento a eliminar */}
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-bold uppercase">Placa / Vehículo:</span>
                  <span className="font-mono font-black text-slate-900 text-sm bg-blue-100 text-blue-900 px-2 py-0.5 rounded">
                    {servicioAEliminar.placa}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-bold uppercase">Chofer / Piloto:</span>
                  <span className="font-semibold text-slate-800">{servicioAEliminar.piloto}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-bold uppercase">Fecha:</span>
                  <span className="font-medium text-slate-700">{servicioAEliminar.fecha}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-500 font-bold uppercase">Tipo Mantenimiento:</span>
                  <span className="font-semibold text-slate-700">
                    {servicioAEliminar.tipoMantenimiento || 'General'}
                  </span>
                </div>
                <div className="pt-1 border-t border-slate-200">
                  <span className="text-slate-500 font-bold uppercase block mb-0.5">Problema Reportado:</span>
                  <p className="text-slate-800 text-[11px] line-clamp-2 italic">
                    {servicioAEliminar.tipoProblema || servicioAEliminar.tipoServicio || servicioAEliminar.descripcion}
                  </p>
                </div>
                <div className="flex items-center justify-between pt-1 border-t border-slate-200">
                  <span className="text-slate-500 font-bold uppercase">Total Registrado:</span>
                  <span className="font-mono font-black text-sm text-emerald-700">
                    {formatQuetzales(servicioAEliminar.costoTotal)}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                ...
                <button
                  ...
                  disabled={eliminando}
                  id="btn-confirmar-eliminar-mantenimiento"
